...
1. Выберите один UDP-пакет. По этому пакету определите, сколько полей содержит UDP-заголовок.
   - всего 4 поля: порт отправителя, порт получателя, длина сообщения и  чексумма
2. Определите длину (в байтах) для каждого поля UDP-заголовка, обращаясь к отображаемой
   информации о содержимом полей в данном пакете.
   - длина каждого поля ровно 2 байта(это видно на скрине справа снизу. аналогично для всех полей)
3. Значение в поле Length (Длина) – это длина чего?
   - длина всего что передаётся в UDP - 4 заголовка по 2 байта и 43 байта нагрузки.
4. Какое максимальное количество байт может быть включено в полезную нагрузку UDP-пакета?
   - Теоретически, максимальный размер поля — 65 535 байт для UDP-датаграммы (8 байт на заголовок и 65 527 на данные). Фактический предел для длины данных при использовании IPv4 — 65 507 (помимо 8 байт на UDP-заголовок требуется ещё 20 на IP-заголовок).
5. Чему равно максимально возможное значение номера порта отправителя?
   - 65535 больше в переменную не влезет
...
7. Проверьте UDP-пакет и ответный UDP-пакет, отправляемый вашим хостом. Определите
   ...
   - 34489 и 53. В другою сторону - те же порты, в другом порядке
...
#### Скрины
Запустил сервер в папку lab06 и перекинул сюда два аудиофайла (потом удалил)
![filezilla.png](images/filezilla.png)
![filezilla2.png](images/filezilla2.png)
...
#### Демонстрация работы
Команды логируются...
todo![server.png](images/server.png)
