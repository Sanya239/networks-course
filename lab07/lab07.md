...
2. Каков IP-адрес у сервера gaia.cs.umass.edu? Каковы номера портов для отправки и приема
   ...
   - 128.119.245.12
   - Наш порт 52252, порт сервера 80
3. Какой порядковый номер у SYN TCP-сегмента, который используется для установления
   ...
   - Номер 785
   - Установлен нужный флаг SYN = 0x2 в Transmission Control Protocol
...
4. Какой порядковый номер у SYNACK-сегмента, отправленного сервером gaia.cs.umass.edu
   ...
   - просто прибавляется 1 к sequence number запроса
   - Флаг 0x12 = SYN + ACK в Transmission Control Protocol
...
6. Рассмотрите TCP-сегмент, содержащий команду POST протокола HTTP, как первый TCP-сегмент 
   ...
   - Номера сегментов 797, 798, 799, 800, 801, 834
   - <!-- todo -->
   - <!-- todo -->
   - примерно 135мс для всех
7. Чему равна пропускная способность (количество байтов, передаваемых в единицу
   ...
   -  Скорость 170Кб/сек
   - Размер пакетв 2896 байт, скорость передачи 0.135сек. Делим одно на другое и вот ответ
...
#### Скрин
![time-graph.png](images/time-graph.png)


# Запуск любых программ в этой лабе
```shell
...
#### Решение
В первом случае клиент не примет все пакеты которые идут после первого недошедшего. Если недошедший пакет имеет номер $i$, то всё будет нормально, если только в соединение не поместится и следующий пакет номер $i$. Таким образом наибольшее подходящее окно будет $k-1$

Во втором случае у нас уже более добрый алгоритм который готов пропустить пару пакетов и подождать побольше. Если размер окна $N$, то в худшем случае $N$ пакетов не дошли и клиент их ждёт, а мы уже отправили ещё $N$. Если $k\leq2N$ то всё пропало. Значит $N\leq k/2$
...
#### Решение
Чтобы заполнение канала было 100%, нужно $0.03\times10^9 = 3e7$бит.

Таким образом, 98% заполнение это $3e7\times0.98=29400000$ байт.

Чтобы узнать размер окна, надо поделить это на количество байт в пакете. Получается $29400000 / 1500 = 19600$. Вот и ответ 