...
todo![upload.png](images/upload.png)
...
todo![download.png](images/download.png)
...
![checksum.png](images/checksum.png)
...
За каждый ход пакета туда-обратно окно увеличивается на 1. Значит $T \times RTT = W/2 \times RTT$.

В то же время, пропускная способность $R$ в момент $t$ это $ R_t = W_{t}\times RTT$, то есть в среднем за время
увеличения получается $R = 3/4W \times RTT$.

Ну подставим: $T\times RTT = W/2\times RTT = 1/2 \times 4/3 R / RTT \times RTT$.

Значит $T = 4/6\times R \times RTT$. Ну вот и доказали, действительно линейная зависимость.

### Задача 2 (3 балла)

Рассмотрим задержку, полученную в фазе медленного старта TCP. Рассмотрим клиент и веб-сервер, напрямую соединенные одним
каналом со скоростью передачи данных $R$.
...
"Игнорируя заголовки протокола". Проигнорируем и начальное рукопожатие, будем считать, что оно уже сделано.

- Начинаем разбирать с третьего случая. Если отправка пакета занимает больше времени чем до нас доходит ACK, то канал
всегда загружен полностью и время которое уйдёт на передачу $15S/R$

- Второй случай: первый пакет уходит и возвращается за RTT времени. Теперь окно уже имеет размер 2. Отправляем 2 пакета
сразу, первый из них возвращается через $RTT$ и сразу начинаем посылать пакеты дальше. Потом следущая волна уже 4
пакета. Успеем послать один и ещё не меньше трёх, пока он летит туда-обратно.

Пусть в задаче равенство, иначе не понятно
что делать. Тогда прежде чем посылать последнюю партию из 8 пакетов, пройдёт ещё RTT времени. Ну и теперь уже мы
тратим $7S/R + RTT$ времени на эти 8 пакетов. Итого $7S/R + 4RTT$

- Первый случай: то же, что и второй, но процесс начинает глохнуть пораньше - уже когда окно 4. Теперь эта передача
занимает не $RTT$, а $4S/R$. Ответ $11S/R + 3RTT$

### Задача 3 (2 балла)

Рассмотрим модификацию алгоритма управления перегрузкой протокола TCP. Вместо
аддитивного увеличения, мы можем использовать мультипликативное увеличение. TCP-отправитель увеличивает размер своего
окна в $(1 + a)$ раз (где $a$ - небольшая положительная
...
желательно обеспечить малое время отклика если конечная система расположена далеко от датацентра, то значение RTT будет
большим, что может привести к неудовлетворительному времени
...
У нас буквально $4RTT$ это один запрос и огронмный handshake.

Если делать рукопожатие с ближним сервером, то
это $3RTT_{FE}$.

Плюс ещё запрос должен долететь до ближднего сервера, потом до дальнего, обработаться и венуться.
Это $RTT_{FE} + RTT_{BE} + {время обработки}$. Ну вот и получилось
