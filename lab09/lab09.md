...
2. Почему ICMP-пакет не обладает номерами исходного и конечного портов?
   - ICMP передаёт сообщение проверки статуса хоста, а не данные для конкретного приложения, поэтому ему и не нужны порты
3. Рассмотрите один из ping-запросов, отправленных вашим хостом. Каковы ICMP-тип и кодовый
   номер этого пакета? Какие еще поля есть в этом ICMP-пакете? Сколько байт приходится на поля 
   контрольной суммы, порядкового номера и идентификатора?
   - Тип echo request номер 0
   - Ещё есть идентификатор, порядковый номер, контрольная сумма и данные
   - все эти поля по 2 байта
4. Рассмотрите соответствующий ping-пакет, полученный в ответ на предыдущий. 
   ...
   - Тип echo reply номер 0
   - Ещё есть идентификатор, порядковый номер, контрольная сумма и данные(timestamp и ещё какие-то 40 байтов)
   - все эти поля по 2 байта
...
2. Рассмотрите на вашем скриншоте ICMP-пакет с сообщением об ошибке. В нем больше полей,
   ...
   - фрейм IP протокола с данными о пути пакета
3. Рассмотрите три последних ICMP-пакета, полученных исходным хостом. Чем эти пакеты
   ...
   - Отличаются тем что у них type не ошибка, а успех
   - Просто ttl хватило до destination
4. Есть ли такой канал, задержка в котором существенно превышает среднее значение? Можете
   ...
   - вот вывод traceroute, шаг 13 долгий
   ```
   1  _gateway (192.168.5.1)  3.358 ms  3.316 ms  3.302 ms
   2  * * *
   3  host-16.177.78.178.ucom.am (178.78.177.16)  6.104 ms  6.093 ms  6.082 ms
   4  host-0.241.48.185.ucom.am (185.48.241.0)  6.068 ms  6.058 ms  6.047 ms
   5  UCOM-LLC.edge1.Sofia2.Level3.net (212.133.10.118)  35.581 ms  35.571 ms  35.535 ms
   6  lag-110.ear1.Sofia2.Level3.net (212.133.10.117)  35.765 ms * *
   7  ae1.3123.ear7.frf1.neo.colt.net (171.75.9.121)  54.853 ms * *
   8  * * *
   9  * * *
   10  * * *
   11  * * *
   12  * * *
   13  101.95.208.38 (101.95.208.38)  389.919 ms *  332.234 ms
   14  * * *
   15  * * *
   16  * * *
   17  * * *
   18  59.82.44.240 (59.82.44.240)  278.275 ms *  406.828 ms
   ```
   - между шагами 7 и 13 прыжок из Болгарии в Китай. Видимо долго...
![taobao2.png](images/taobao2.png)
...
#### Демонстрация работы
todo![my_addr.png](images/my_addr.png)
...
#### Демонстрация работы
![ip_range.png](images/ip_range.png)
...
#### Демонстрация работы  
![my_copies.png](images/my_copies.png)
...
#### Решение
1. Пакет потерян близко к концу передачи, поэтому эта потеря не оказала влияния на скопрость роста соединения. Средняя скорость роста составляет $3/4W$. Время передачи составило $W/2+1$ (+1 из-за потерянного пакета).

Тогда всего было отправлено $3/8W^2 + 3/4W$. Получается $L = \dfrac{1}{\frac{3}{8} W^2 + \frac{3}{4} W}$.

2. Пусть $W^2 >> W$. Тогда $3/8W^2 + 3/4W \approx 3/8W^2$. Значит $W \approx \sqrt{8L/3}$.

Средняя скорость это $Avg = W_{avg}MSS/RTT$. Как мы знаем, $W_{avg} = 3/4W$. Получается $Avg = 3/4W \cdot MSS/RTT = 1.22\sqrt{L}\cdot MSS/RTT$
...
#### Решение
В мультипликативном увеличении принято сокращать окно в 2 раза после неудачного пакета. Это значит, что стабильная скорость передачи устроена так, что $a^n(W/2) = W$ где n количество удачных пакетов, перед неудачным. $L = 1/(n+1) = 1/(ln_a(2)+1)$.(тут L это доля потерянных пакетов)

Ну значит частота потери пакетов это $L = W/ (ln_a(2)+1)$
2. Кажется очевидно, что средняя пропускная способность всегда сразу сокращается и время роста нагрузки просто $ln_a(2)$