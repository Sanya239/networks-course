...
4. Отсортируйте отслеженные пакеты по их исходному IP-адресу; для этого щелкните по
   ...
   2. Какие поля не меняются? Какие поля должны оставаться неизменными? Какие
      ...
      - Не меняются source addr, dest addr, protocol, ICMP Type
      - Меняются: чексумма, TTL, sequence number, чексумма
...
6. Остаются ли эти значения (Identification и TTL) неизменными во всех сообщениях
   ...
   - Identification изменяется, TTL - нет
...
8. **Фрагментация**. Остановите захват пакетов в Wireshark. Измените размер пакета
   ...
   1. Было ли это сообщение фрагментировано между двумя или более IP-дейтаграммами? 
      ...
      - Развалилось на 3 фрагмента
   2. Какие поля IP-заголовка изменяются в разных фрагментах?
      - Чексумма


![wireshark0.png](images/wireshark0.png)
![wireshark1.png](images/wireshark1.png)
![wireshark2.png](images/wireshark2.png)
...
На картинке доступ к серверу в Австралии, серверу в Африке, подсчёт статистики и вывод ошибки о таймауте
### Задание Б (1 балл)
...
#### Демонстрация работы
![icmp2.png](images/icmp2.png)
Результат выключения сети во время работы программы
